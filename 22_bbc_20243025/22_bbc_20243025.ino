...
#define _SERVO_ANGLE_DIFF  53  // Replace with |D - E| degree
#define _SERVO_SPEED       55  // servo speed
...
void setup()
{
  ...
  duty_change_per_interval = (float)(_DUTY_MAX - _DUTY_MIN) * ((float)_SERVO_SPEED / _SERVO_ANGLE_DIFF) * (_INTERVAL_SERVO / 1000.0); 
}
...
void loop()
{
  ...
  if (event_serial) {
    ...
    //Serial.print("duty_change: "); Serial.println(duty_change_per_interval);
  }
}
...
float volt_to_distance(int a_value)
{
  ...
  //float volt = (float)a_value;
  //return 270.0 - (0.2 * volt); // 선형화된 거리 계산
  //return ???;
}
